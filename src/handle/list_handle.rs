...
pub async fn list_api(request: HttpRequest, params: web::Query<HashMap<String,String>>) -> HttpResponse {
    ...
    if !auth_result{
        println!("验证[不]通过:{}",msg);
        return HttpResponse::Ok().json(structs::ResultBuild::<i8>::fail_with_msg(msg));
    }
    ...
    if file_dir.is_none(){
        return HttpResponse::Ok().json(structs::ResultBuild::<&str>::fail_with_msg("参数缺失[file_dir]"));
    }
    ...
    if !file_path_object.exists() {
        ...
        println!("{}",msg);
        return HttpResponse::Ok().json(structs::ResultBuild::<&str>::fail_with_msg(msg.as_str()));
    }
    ...
    HttpResponse::Ok().json(structs::ResultBuild::success_with_data(user_file_list))
