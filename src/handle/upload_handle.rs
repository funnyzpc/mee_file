...
pub async fn upload(mut payload: Multipart, request: HttpRequest) -> Result<HttpResponse, Error> {
    ...
    if !auth_result{
        println!("验证[不]通过:{}",msg);
        return Ok(HttpResponse::Ok().json(structs::ResultBuild::<i8>::fail_with_msg(msg)));
    }
    ...
    while let Ok(Some(mut field)) = payload.try_next().await {
        ...
        if content_name==Some(""){
            return Ok(HttpResponse::Ok().json(structs::ResultBuild::<i8>::fail_with_msg("KEY缺失[1]")));
        }
        println!("content_name:{}",content_name.unwrap());
        let filename = content_type.get_filename().unwrap();
        println!("filename:{},content_name:{}",filename,content_name.unwrap());
        if filename==""{
            return Ok(HttpResponse::Ok().json(structs::ResultBuild::<String>::fail_with_msg("文件名缺失[2]")));
        }
    ...
    Ok(HttpResponse::Ok().json(structs::ResultBuild::success_with_data(user_file_list)))
}
...
pub async fn upload_api(mut payload: Multipart, request: HttpRequest) -> Result<HttpResponse, Error> {
    ...
    while let Ok(Some(mut field)) = payload.try_next().await {
        ...
        let content_name = content_type.get_name();
        if content_name==Some(""){
            return Ok(HttpResponse::Ok().json(structs::ResultBuild::<String>::fail_with_msg("KEY缺失[1]")));
        }
        let filename = content_type.get_filename().unwrap();
        if filename==""{
            return Ok(HttpResponse::Ok().json(structs::ResultBuild::<String>::fail_with_msg("文件名缺失[2]")));
